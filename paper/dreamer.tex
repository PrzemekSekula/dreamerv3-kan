...
Game & \shortstack{DreamerV3\\(baseline)} & \shortstack{Sparse-KAN\\(ours)}
   & Game & \shortstack{DreamerV3\\(baseline)} & \shortstack{Sparse-KAN\\(ours)} \\
\midrule
Alien          & \textbf{0.150} & 0.086 & Kangaroo       & \textbf{1.355} & 0.344 \\
Amidar         & \textbf{0.086} & 0.044 & Krull          & \textbf{6.177} & 6.123 \\
Assault        & \textbf{1.060} & 0.885 & Kung Fu Master & 1.138 & \textbf{1.277} \\
Asterix        & \textbf{0.111} & 0.105 & Ms.\ Pac-Man   & \textbf{0.279} & 0.099 \\
Bank Heist     & \textbf{1.231} & 1.058 & Pong           & \textbf{1.093} & 1.025 \\
Battle Zone    & \textbf{0.215} & 0.062 & Private Eye    & \textbf{0.001} & \textbf{0.001} \\
Boxing         & \textbf{6.905} & 6.332 & Qbert          & \textbf{0.182} & 0.025 \\
Breakout       & \textbf{0.938} & 0.456 & Road Runner    & \textbf{1.870} & 1.292 \\
Chopper Cmd.   & \textbf{0.199} & 0.090 & Seaquest       & \textbf{0.014} & 0.009 \\
Crazy Climber  & \textbf{2.790} & 2.302 & Up N Down      & 0.942 & \textbf{1.880} \\
Demon Attack   & \textbf{0.048} & 0.036 & & & \\
Freeway        & \textbf{0.216} & 0.013 & & & \\
Frostbite      & \textbf{0.561} & 0.388 & & & \\
Gopher         & \textbf{1.118} & 1.005 & & & \\
Hero           & \textbf{0.169} & 0.071 & & & \\
Jamesbond      & \textbf{1.542} & 1.510 & & & \\
\midrule
\multicolumn{4}{l}{Mean HNS}   & \textbf{1.169} & 1.020 \\
\multicolumn{4}{l}{Median HNS} & \textbf{0.750} & 0.366 \\
\multicolumn{4}{l}{Games $>$ human} & \textbf{11} & 10 \\
\bottomrule
\end{tabular}
\end{table}
...
score distributions that we do not have for all baselines, we report plain mean and
median HNS for all methods.  Published values for the literature baselines are taken
directly from Table~A.1 of~\cite{schwarzer2023bbf}.

\begin{table}[ht]
\centering
\caption{Aggregate human-normalized scores on Atari~100k (26 games).
  Mean and median HNS for literature methods are taken from~\cite{schwarzer2023bbf};
  values for our two conditions are computed from the final training checkpoint
  averaged across seeds.  Higher is better.  Methods above the divider are
  model-free; below are model-based.}
\label{tab:aggregate}
\begin{tabular}{lcc}
\toprule
Method & Median HNS & Mean HNS \\
\midrule
Random          & 0.000 & 0.000 \\
DER             & 0.189 & 0.350 \\
DrQ($\epsilon$) & 0.313 & 0.465 \\
SPR             & 0.396 & 0.616 \\
SR-SPR          & 0.685 & 1.272 \\
\midrule
IRIS            & 0.289 & 1.046 \\
EfficientZero   & 1.116 & 1.945 \\
BBF             & 0.917 & 2.247 \\
DreamerV3 (baseline, ours) & 0.750 & 1.169 \\
\textbf{Sparse-KAN (ours)} & \textbf{0.366} & \textbf{1.020} \\
\midrule
Human           & 1.000 & 1.000 \\
\bottomrule
\end{tabular}
...
achieving super-human performance on 10 of them.  A median HNS of~0.37 places it
between DrQ($\epsilon$) and SR-SPR, two strong model-free baselines, even though
Sparse-KAN operates on top of a world model trained with far fewer environment
interactions than those methods typically require for comparably rich representations.
The mean HNS of~1.02 is inflated by games where both conditions dramatically exceed
human level (most notably Boxing and Krull), but the breadth of learning---positive
returns on every game---confirms that the KAN-based actor-critic is a viable function
approximator in the DreamerV3 imagination rollout loop.

\paragraph{Comparison with the MLP baseline.}
Sparse-KAN underperforms the DreamerV3 MLP baseline (median~0.37 vs.~0.75), a gap
...
representations where splines are beneficial.  We therefore view the 0.37 median HNS
